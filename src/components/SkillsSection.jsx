...
const SkillsSection = () => {
  ...
  return (
    <div className="bg-slate-950">
      {/* Main wrapper */}
      <div ref={wrapperRef} className="w-full h-[100svh] md:h-screen flex items-center justify-center relative overflow-hidden">
        {/* Background effects */}
        <div className="absolute inset-0 bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950"></div>
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-blue-900/20 via-transparent to-transparent"></div>

        {/* Title */}
        <div className="absolute top-16 md:top-20 left-1/2 -translate-x-1/2 z-10 px-4">
          <h2 className="text-4xl sm:text-6xl md:text-8xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-blue-400 via-purple-500 to-pink-500">
            My Skills
          </h2>
        </div>
        ...
        <div 
          ref={sliderRef}
          className="relative w-[92vw] max-w-md md:max-w-2xl h-[60vh] max-h-[520px]"
          style={{ perspective: "1000px" }}
        >
          {/* Dashed border outline */}
          <div className="absolute -inset-4 md:-inset-8 border-2 border-dashed border-gray-700/30 rounded-3xl pointer-events-none"></div>
          ...
          {skillSlides.map((slide, index) => (
            ...
            >
              ...
              {/* Content */}
              <div className="relative h-full flex flex-col items-center justify-center p-6 md:p-8 text-white">
                {/* Icon */}
                <div className="mb-5 md:mb-6 p-3 md:p-4 bg-white/20 backdrop-blur-sm rounded-2xl">
                  {slide.icon}
                </div>

                {/* Title */}
                <h3 className="text-3xl md:text-5xl font-bold mb-6 md:mb-8 text-center drop-shadow-lg">
                  {slide.title}
                </h3>

                {/* Skills */}
                <div className="flex flex-wrap justify-center gap-2 md:gap-3 max-w-[85%] sm:max-w-xs md:max-w-lg">
                  {slide.skills.map((skill, idx) => (
                    <div
                      key={idx}
                      className="px-3 md:px-4 py-1.5 md:py-2 bg-white/20 backdrop-blur-md rounded-full text-xs sm:text-sm md:text-base font-medium border border-white/30 hover:bg-white/30 transition-all duration-300 hover:scale-105"
                    >
                      {skill}
                    </div>
                  ))}
                ...
                {/* Slide counter */}
                <div className="absolute bottom-4 md:bottom-6 right-4 md:right-6 text-white/60 text-xs md:text-sm font-medium">
